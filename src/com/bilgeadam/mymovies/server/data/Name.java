package com.bilgeadam.mymovies.server.data;
...
import com.bilgeadam.mymovies.utils.Props;
...
public class Name implements Externalizable {
	...
	private static Name parse(String line) {
		...
		name.id          = tokenizer.nextToken(); // nconst
		name.primaryName = tokenizer.nextToken(); // primaryName

		String token = tokenizer.nextToken(); // birthYear
		if (token.equalsIgnoreCase("\\N")) {
			name.birthYear = CommonData.UNSPECIFIED;
		}
		else {
		...
		if (token.equalsIgnoreCase("\\N")) {
			...
			StringTokenizer professions    = new StringTokenizer(token, ",");  //actor,producer,soundtrack
	...
	static public String[] MovieIds(String actorName) {
		...
		if (name != null) {
			retValue = name.knownForTitles; // tt0050419,tt0031983,tt0053137,tt0072308 gibi bir liste geliyor e�er akt�r
											// tanımlı ise
		}
		return retValue;
	}
